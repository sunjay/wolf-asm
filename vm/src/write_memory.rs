...
impl<T: WriteMemory> WriteMemory for [T] {
    fn write_into(&self, mem: &mut Memory, mut addr: usize) -> Result<usize, OutOfBounds> {
        for value in self {
...
impl WriteMemory for exec::StaticUninit {
    fn write_into(&self, _mem: &mut Memory, addr: usize) -> Result<usize, OutOfBounds> {
        let &Self {nbytes} = self;
        Ok(addr + mem::size_of::<u8>() * nbytes as usize)
    }
}
